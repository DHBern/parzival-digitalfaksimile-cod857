...
function setListeners() {
	...
	infoButton.addEventListener("click", function() {
  		infoFenster('info/infoNibelungen.html');
	});
...
function checkSeiteUp() {
		...
		if (curSlide==417) {
			if (zoom==50) {
			window.location.href="klage.html?415&50";
			curSlide=415;
			}
			if (zoom==100 || zoom==150) {
			window.location.href="klage.html?416" + "&" + zoom; 
			}
		}
...
if ((curSlide % 2)!=0 && curSlide>1 && zoom==50) {curSlide--;}
showSlide();
}
}
