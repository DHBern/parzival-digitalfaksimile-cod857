...
function setListeners() {
	...
	infoButton.addEventListener("click", function() {
  		infoFenster('info/infoKarl.html');
	});
...
function showSlide() {
	if (browserOK) {
		...
		if(curSlide==560 && zoom!=50) {
		document.title="St. Gallen, Stiftsbibl., Cod. 857 - Willehalm";
		}
		
		if(curSlide!=560 || (curSlide==560 && zoom==50)) {
		document.title="St. Gallen, Stiftsbibl., Cod. 857 - Karl der Gro" +sz+ "e";
		}
		...
		if(curSlide>708 && curSlide!=1456 && curSlide!=1457) {
...
function goNext() {
	...
	if (zoom == 50) {
		...
		if(curSlide==455 || curSlide==456) {
		curSlide=1456;
		showSlide();
		return;
		}
		
		if(curSlide==1456 || curSlide==1457) {
		curSlide=457;
		showSlide();
		return;
		}	
...
function zoom150() {
		...
		if(curSlide==560 || curSlide==559) {seitenEingabeAnpassen()}
		
		if (curSlide<=452 && zoom==50){window.location.href="klage.html?451&150";}
		if(zoom==50 && curSlide<707) {seiteAnpassen();}
		zoom = 150;
		showSlide();	
}
...
function zoom100() {
		...
		if (curSlide<=452 && zoom==50){window.location.href="klage.html?451&100";}
		
		if(curSlide==456 && zoom==50) {
		curSlide=455;
		}
		if(zoom==50 && curSlide<707) {seiteAnpassen();}
		zoom = 100;
...
function checkSeiteDown() {
		...
		if ((curSlide==449 || curSlide==450) && zoom==50) {
		window.location.href="klage.html?451";
		curSlide=451;
		}
...
if ((curSlide % 2)==0 && curSlide>415 && curSlide<707 &&zoom==50) {curSlide--;}
showSlide();
}
}